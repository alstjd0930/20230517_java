...
SELECT *
FROM employee
where hire_date between to_date(19900909,'yyyymmdd') and to_date(20010101,'yyyymmdd');
...
FROM employee;
--17.
select emp_name, abs(floor(sysdate-hire_date)) 근뭉일수1, abs(floor(hire_date-sysdate)) 근뭉일수1
from employee
;
--18
select emp_id
...
--21
--22
select emp_id,emp_name,dept_code,case when dept_code='D5' then '총무부' when dept_code='D6' then '기획부' when dept_code='D9' then '영업부' end 처리
from employee
where dept_code in ('D5','D6','D9')
order by dept_code desc
;
--23
select emp_name,substr(emp_no,8,12),substr(emp_no,1,6),substr(emp_no,8,12)+substr(emp_no,1,6)
from employee
;
--24
select salary*12*nvl(bonus,1)
from employee
